...
resolves the ones blocking implementation and defers the rest. The
biases here are starting points — record the chosen answer + one-
line *why* under each in **this** file during stage 1.

1. **OQ-WASM-1 — runtime-adapter table placement.**
   Bias: table in `codeless-tools` (mobile-safe), adapters in
   `codeless-plugin-host-wasm` (host-only). Confirms the table
   itself uses no host-only types; if a type leak emerges, the
   fallback is "table moves to a new mobile-safe `codeless-plugin-
   dispatch` crate."
2. **OQ-WASM-2 — wit-bindgen output: commit or build.rs.**
   Bias: commit. Reviewing a WIT change and its bindings in the
   same PR is the point; hiding them behind `build.rs` makes
   reviewer archaeology worse.
3. **OQ-WASM-4 — hot reload on migration change.**
   Bias: refuse (with structured error pointing at restart). Dev
   ergonomics are nice but migration replay in a running server is
   a separate piece of design.
4. **OQ-WASM-5 — fuel/memory/deadline knobs surface.**
   Bias: global defaults in `HostPolicy`; per-plugin overrides via
   codeless config (`plugins.<id>.fuel = …`). The plugin manifest
   cannot raise its own limits.
5. **OQ-MCP-1 — `mcp_forward` in v0.1.**
   Bias: defer to v0.2. v0.1 ships `tool_call` + `rest_proxy`;
   `mcp_forward` is parsed but produces `Failed`.
6. **OQ-UI-1 — slot vocabulary versioning.**
   Bias: `@codeless/plugin-ui-sdk` package version pins the slot
   vocabulary. The host refuses to mount a plugin whose declared
   SDK version is newer than the host's.
7. **Slot vocabulary final set.** v0.1 ships five slots
   (`assistant-panel`, `tool-result:<tool_id>`,
   `persona-picker:<persona_id>`, `settings-page:<plugin_id>`,
   `composer-attachment-action:<plugin_id>`). Stage 1 may shrink
   the set; growing it is a separate decision per the doc
   ("adding a slot is a host-side change"). Record the final v0.1
   set in `PLUGIN-UI-FEDERATION.md` "Slot vocabulary" before any
   host code lands.
