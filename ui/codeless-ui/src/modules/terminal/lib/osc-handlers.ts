...
export type CodelessOpenInput = {
  file: string;
};

export function registerCodelessOpenHandler(
  term: Terminal,
  onCodelessOpen: (input: CodelessOpenInput) => void,
): () => void {
  const d = term.parser.registerOscHandler(8888, (data) => {
    const input = parseCodelessOpen(data);
    if (input) onCodelessOpen(input);
    return true;
  });
  return () => d.dispose();
}
...
function parseCodelessOpen(data: string): CodelessOpenInput | null {
