...
function ensureSession(leafId: number, initialCwd?: string): Session {
  ...
  session.ready = (async () => {
    ...
    session.cleanups.push(
      ...
      registerCodelessOpenHandler(term, (input) => {
        session.callbacks.onCodelessOpen?.(input);
      }),
    );
  })();
  ...
  useEffect(() => {
    ...
    s.ready.then(() => {
      ...
      attachSession(leafId, container.current, {
        ...
        onCodelessOpen: (input) => cbRef.current.onCodelessOpen?.(input),
      });
