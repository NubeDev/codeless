...
import { type CodelessOpenInput } from "./lib/useTerminalSession";
...
type Props = {
  ...
  onCodelessOpen?: (leafId: number, input: CodelessOpenInput) => void;
...
type Bundle = {
  ...
  onCodelessOpen: (input: CodelessOpenInput) => void;
};
...
export function TerminalStack({
  ...
  const codelessOpenRef = useRef(onCodelessOpen);
  useEffect(() => {
  ...
  useEffect(() => {
    codelessOpenRef.current = onCodelessOpen;
  }, [onCodelessOpen]);
  ...
  const getBundle = (leafId: number): Bundle => {
    ...
    if (!b) {
      b = {
        ...
        onCodelessOpen: (input) => codelessOpenRef.current?.(leafId, input),
      };
