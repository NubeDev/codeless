...
export function AiComposerProvider({ children }: ProviderProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener("codeless:ai-attach-file", onAttach);
    return () => window.removeEventListener("codeless:ai-attach-file", onAttach);
  ...
  const submit = () => {
    ...
    // the prompt to the CODELESS.md scan template before sending.
    ...
    if (commandSource.startsWith("/") || commandSource.startsWith("#")) {
      ...
      if (outcome.kind === "send-prompt") {
        ...
        if (outcome.commandName) {
          commandMarker = `<codeless-command name="${outcome.commandName}" />`;
        }
      }
    }
