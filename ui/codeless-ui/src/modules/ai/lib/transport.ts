...
async function readCodelessMd(workspaceRoot: string | null): Promise<string | null> {
  if (!workspaceRoot) return null;
  const path = `${workspaceRoot.replace(/\/$/, "")}/CODELESS.md`;
  ...
  try {
    ...
    const content =
      r.content.length > CODELESS_MD_MAX_BYTES
        ? r.content.slice(0, CODELESS_MD_MAX_BYTES)
        : r.content;
